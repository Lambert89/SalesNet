...
public class NavigationOrchistration {
    ...
    public @ResponseBody
    String subscribe(@RequestBody final Map<String, String> body) throws MessagingException {
        String result = "true";
        ...
        if (StringUtils.isBlank(contactMeIndicator) && StringUtils.isNotBlank(phoneNumber)) {
            throw new ValidationException("phoneNumber&contactMeIndicator");
        }
        if (StringUtils.isNotBlank(contactMeIndicator) && StringUtils.isBlank(phoneNumber)) {
            throw new ValidationException("phoneNumber&contactMeIndicator");
        }
        final User user = new User();
        user.setMail(mailAddress);
        user.setName(userName);
        if (StringUtils.isNotBlank(contactMeIndicator) && StringUtils.isNotBlank(phoneNumber)) {
            user.setContactMeIndicator(true);
            user.setPhoneNumber(phoneNumber);
        }
        try {
            final User exist = this.persistenceService.findObject(User.class, body.get("mailAddress"));
            if (exist == null) {
    ...
    public @ResponseBody
    Map<String, Object> handleValidationError(final ValidationException exception, final HttpServletRequest request) {
